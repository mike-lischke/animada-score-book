...
describe("TrackClipboard", () => {
    ...
    const makeNoteStyle = (id: string): IAudioData => {
        return {
            id,
            symbol: undefined,
            audioBuffer: null,
    ...
    beforeEach(() => {
        ...
        const notes: ISbDmNoteEvent[] = new Array(10).fill(null).map((_, i) => {
            return {
                ...
                audioData: makeNoteStyle(String(i + 1)),
            };
        });

        // Insert a rest at step 6
        notes[5].audioData = undefined;
    ...
    it.skip("retains original copied NoteStyles even if track has changed", () => {
        ...
        mockTrack.notes[1].audioData = newNoteStyle;
