...
const unpackPolyrhythmString = (packedPolyrhythmsString: string): string => {
...
const deserialiseNotes = (serialisedNotes: string, instrumentId: string, trackNoteCount: number): string[] => {
    const notesAsNumber = urlDecodeNumber(serialisedNotes);
    const base = BigInt(getNoteStyleCount(instrumentId));
