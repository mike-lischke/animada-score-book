...
const handleHealth = async (_req: IncomingMessage, res: ServerResponse): Promise<void> => {
    ...
    if (auth.adapter.isInitialized()) {
        try {
            const rows = await auth.adapter.query<{ cnt: number; }>(
    ...
    sendJson(res, {
        status: "ok",
        configLoaded: true,
        initialized: auth.adapter.isInitialized(),
...
const handleSetup = async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
    ...
    if (overwrite && auth.adapter.isInitialized()) {
        ...
        if (usersExist) {
            ...
            if (!user || !(await auth.isUserInAdminGroup(user.userId))) {
...
const handleListScoreFolderContent = async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
    ...
    const folders = await auth.adapter.query(
        `SELECT f.*, (
    ...
    const scores = await auth.adapter.query(
    ...
    for (const f of folders) {
        const summary = await auth.getPermissionSummary(user, "folder", f.id as number);
    ...
    for (const s of scores) {
        const summary = await auth.getPermissionSummary(user, "score", s.id as number);
...
const handleGetScore = async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
    ...
    const rows = await auth.adapter.query(
    ...
    const score = rows[0];
    const summary = await auth.getPermissionSummary(user, "score", score.id as number);
...
const handleResetChildPermissions = async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
    ...
    const collectChildren = async (parentId: number): Promise<void> => {
        const subFolders = await auth.adapter.query<{ id: number; }>(
        ...
        const scores = await auth.adapter.query<{ id: number; }>(
...
const handleAddScoreFolder = async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
    ...
    const result = await auth.adapter.insertReturningId(
...
const handleRenameEntry = async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
    ...
    const allowed = await auth.checkPermission(user, type, id, AccessLevel.Write);
    ...
    await auth.adapter.execute(`UPDATE ${table} SET name = ? WHERE id = ?`, [name, id]);
...
const handleUpdateScore = async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
    ...
    const allowed = await auth.checkPermission(user, "score", id, AccessLevel.Write);
    ...
    await auth.adapter.execute("UPDATE scores SET content = ? WHERE id = ?", [content, id]);
...
const handleDelete = async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
    ...
    if (type === "score") {
        await auth.adapter.execute("DELETE FROM scores WHERE id = ?", [id]);
    ...
    if (type === "folder") {
        const folders = await auth.adapter.query<{ parentid: number | null; }>(
        ...
        await auth.adapter.execute("DELETE FROM folders WHERE id = ?", [id]);
        // Also delete the permission and entity_groups entries.
        await auth.adapter.execute(
...
const handleMove = async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
    ...
    if (type === "folder") {
        ...
        await auth.adapter.execute("UPDATE folders SET parentid = ? WHERE id = ?", [
    ...
    if (type === "score") {
        ...
        await auth.adapter.execute("UPDATE scores SET folderid = ? WHERE id = ?", [
...
const handleClearAll = async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
    ...
    await auth.adapter.execute("DELETE FROM scores");
    await auth.adapter.execute("DELETE FROM folders");
...
const handleLogin = async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
    ...
    const admin = await auth.isUserInAdminGroup(user.id);
    ...
    const capabilities = await auth.buildCapabilities(payload);
...
const handleGroupLogin = async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
    ...
    const anonRows = await auth.adapter.query<{ id: number; username: string; displayName: string; }>(
...
const handleRefresh = async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
    ...
    const result = await auth.verifyAndRotateRefreshToken(rawToken);
    ...
    const rows = await auth.adapter.query<{ id: number; username: string; }>(
...
const handleWhoAmI = async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
    ...
    if (!user) {
        const capabilities = await auth.buildCapabilities(undefined);
    ...
    const dbUser = rows[0];
    const admin = await auth.isUserInAdminGroup(dbUser.id);
    const payload: ITokenPayload = {
        userId: dbUser.id,
    ...
    if (user.authType === "group" && user.groupId !== undefined) {
        const groupRows = await auth.adapter.query<{ id: number; name: string; }>(
...
const handleCreateInitialAdmin = async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
    ...
    const adminGroupResult = await auth.adapter.insertReturningId(
    ...
    await auth.adapter.insertReturningId(
    ...
    const defaultGroupRows = await auth.adapter.query<{ id: number; }>(
    ...
    if (defaultGroupRows.length > 0) {
        ...
        const agResult = await auth.adapter.insertReturningId(
            "INSERT INTO `groups` (name, description, color) VALUES (?, ?, ?)",
            [groupName, "", "#2a9d8f"],
        );
    ...
    for (const f of orphanFolders) {
        await auth.setOwner("folder", f.id, result.insertId);

        if (worldId !== undefined) {
            await auth.addEntityGroup("folder", f.id, worldId, false);
        }

        if (defaultGroupId) {
            await auth.addEntityGroup("folder", f.id, defaultGroupId, false);
        }
    }

    // Scores without permissions.
    const orphanScores = await auth.adapter.query<{ id: number; }>(
    ...
    for (const s of orphanScores) {
        await auth.setOwner("score", s.id, result.insertId);

        if (worldId !== undefined) {
            await auth.addEntityGroup("score", s.id, worldId, false);
        }

        if (defaultGroupId) {
            await auth.addEntityGroup("score", s.id, defaultGroupId, false);
        }
    }
...
const handleCreateUser = async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
    ...
    if (!authUser || !(await auth.isUserInAdminGroup(authUser.userId))) {
        sendError(res, "Forbidden", 403);
    ...
    const groupCollision = await auth.adapter.query<{ cnt: number; }>(
...
const handleDeleteUser = async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
    ...
    if (id === authUser.userId) {
        const adminCount = await auth.adapter.query<{ cnt: number; }>(
    ...
    await auth.adapter.execute("DELETE FROM users WHERE id = ?", [id]);
...
const handleCreateGroup = async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
    ...
    const userCollision = await auth.adapter.query<{ cnt: number; }>(
        "SELECT COUNT(*) AS cnt FROM users WHERE username = ?",
        [name],
    );
...
const handleUpdateGroup = async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
    ...
    const groupRow = await auth.adapter.query<{ admin_id: number | null; }>(
    ...
    if (!user || (!(await auth.isUserInAdminGroup(user.userId)) && !isGroupAdmin)) {
        sendError(res, "Forbidden", 403);
    ...
    const groupInfo = await auth.adapter.query<{ name: string; }>(
    ...
    if (adminId !== undefined) {
        if (!await auth.isUserInAdminGroup(user.userId)) {
    ...
    if (password !== undefined) {
        ...
        if (groupInfo[0]?.name === Auth.adminGroupName && password !== null) {
...
const handleDeleteGroup = async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
    ...
    await auth.adapter.execute("DELETE FROM `groups` WHERE id = ?", [id]);
...
const handleListGroupMembers = async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
    ...
    if (!authUser || (!(await auth.isUserInAdminGroup(authUser.userId)) && !isGroupAdmin)) {
        sendError(res, "Forbidden", 403);
...
const handleGetPermissions = async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
    ...
    const resolvedOwner = await auth.getExplicitOwner(entityType, entityId);
    ...
    const groups = await auth.getExplicitEntityGroups(entityType, entityId);
...
const handleSetPermissions = async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
    ...
    const isAdmin = await auth.isUserInAdminGroup(user.userId);
    const resolvedOwner = await auth.getExplicitOwner(entityType, entityId);
    ...
    if (body.ownerId !== undefined) {
        ...
        await auth.setOwner(entityType, entityId, newOwnerId);
    }

    // Add groups if provided.
    if (Array.isArray(body.addGroups)) {
        for (const g of body.addGroups as Array<{ groupId: number; writable: boolean; }>) {
            await auth.addEntityGroup(entityType, entityId, g.groupId, g.writable);
        }
    }
    ...
    if (Array.isArray(body.removeGroups)) {
        for (const g of body.removeGroups as Array<{ groupId: number; }>) {
            await auth.removeEntityGroup(entityType, entityId, g.groupId);
        }
    }
